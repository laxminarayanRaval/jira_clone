from app.models import Project, User, Issue, Comment, SessionLocal
...
def data_seeder():
    ...
    models = [Project, User, Issue, Comment]
    counts = {f"{i.__name__}_count": db.query(i).count() for i in models}

    print(
        counts,
        f"\n--> All Tables are{' ' if all(counts) else ' not '}Empty, Need to Seed.",
    )

    if not counts.get("Project_count", 0):
        print("seeding data into project table")
        projects_lst = [
            Project(
                name="Jira Clone 1.0",
                url="https://github.com/laxminarayanRaval/jira_clone",
        ...
        db.add_all(projects_lst)
        db.commit()

    if not counts.get("User_count", 0):
        print("seeding data into user table")
        users_lst = [
            User(
                name="Laxminarayan",
                email="lx@jira.guest",
                avatarUrl="https://i.pravatar.cc/150?img=1",
                projectId=projects_lst[0].id,
            ),
            User(
                name="Shimbham Singh Rajput",
                email="ssr@jira.guest",
        ...
        db.add_all(users_lst)
        db.commit()

    if not counts.get("Issue_count", 0):
        print("seeding data into issue table")
        issues_lst = [
            Issue(
            ...
            Issue(
                ...
                listPosition=4,
                description=f"<h1><span style='color: rgb(51, 51, 51);'>🍏 🍎 🍐 🍊 🍋 🍌 🍉 🍇 🍓 🍈 🍒 🍑 🍍 🥭 🥥 🥝 🍅 🍆 🥑 🥦 🥒 🥬 🌶 🌽 🥕 🥔 🍠 🥐 🍞 🥖 🥨 🥯 🧀 🥚 🍳 🥞 🥓 🥩 🍗 🍖 🌭 🍔 🍟 🍕 🥪 🥙 🌮 🌯 🥗 🥘 🥫 🍝 🍜 🍲 🍛 🍣 🍱 🥟 🍤 🍙 🍚 🍘 🍥 🥮 🥠 🍢 🍡 🍧 🍨 🍦 🥧 🍰 🎂 🍮 🍭 🍬 🍫 🍿 🧂 🍩 🍪 🌰 🥜 🍯 🥛 🍼 ☕️ 🍵 🥤 🍶 🍺 🍻 🥂 🍷 🥃 🍸 🍹 🍾 🥄 🍴 🍽 🥣 🥡 🥢</span></h1>",
                estimate=4,
                timeSpent=4,
                reporterId=users_lst[0].id,
                project=projects_lst[0],
                users=[users_lst[2]],
            ),
            Issue(
            ...
            Issue(
                ...
                priority=IssuePriority.HIGH,
                listPosition=6,
                description=f"<h2>Try assigning <u style='background-color: rgb(204, 232, 204);'>Pickle Rick</u> to this issue. <span style='color: rgb(51, 51, 51);'>🥒&nbsp;🥒&nbsp;🥒</span></h2><p><br></p>",
                estimate=6,
                timeSpent=3,
                reporterId=users_lst[1].id,
                project=projects_lst[0],
                users=[users_lst[1], users_lst[2]],
            ),
            Issue(
        ...
        db.add_all(issues_lst)
        db.commit()

    if not counts.get("Comment_count", 0):
        print("seeding data into comment table")
        comments_lst = [
            Comment(
                body="An old silent pond...\nA frog jumps into the pond,\nsplash! Silence again.",
                issueId=issues_lst[0].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="Autumn moonlight-\na worm digs silently\ninto the chestnut.",
                issueId=issues_lst[1].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="In the twilight rain\nthese brilliant-hued hibiscus -\nA lovely sunset.",
                issueId=issues_lst[2].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="A summer river being crossed\nhow pleasing\nwith sandals in my hands!",
                issueId=issues_lst[3].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="Light of the moon\nMoves west, flowers' shadows\nCreep eastward.",
                issueId=issues_lst[4].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="In the moonlight,\nThe color and scent of the wisteria\nSeems far away.",
                issueId=issues_lst[5].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="O snail\nClimb Mount Fuji,\nBut slowly, slowly!",
                issueId=issues_lst[6].id,
                userId=users_lst[2].id,
            ),
            Comment(
                body="Everything I touch\nwith tenderness, alas,\npricks like a bramble.",
                issueId=issues_lst[7].id,
                userId=users_lst[2].id,
            ),
        ]

        db.add_all(comments_lst)
        db.commit()
